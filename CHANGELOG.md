...
- QMD and project memory identity follow the Git primary checkout. Workbench searches the shared `pi-workbench-state` and `pi-workbench-project` collections so learnings are not split across hashed checkout names. A worktree that already had path-hashed collections keeps those names as extra search targets.
