...
Pi's built-in catalog may not list `grok-4.7` yet. When Main Pi asks for it and the registry does not have it, Workbench merges that one model into the Pi agent directory's `models.json` and reloads the xAI provider. Built-in models stay. No API key is written; use `/login` if the switch says the provider has no credential. A symlink or invalid `models.json` is left unchanged.
