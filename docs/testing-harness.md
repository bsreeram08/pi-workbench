...
For visual work, run `/plan --visual`. Submit the seven-field `designBrief` (including `references` and `refusals`) during plan review; a packet without `runtime-observation` and `artifact-inspection` must fail in the host. At execution, source evidence alone must not permit verify. Visual plans must use `visual` with `captureUrl` on loopback; a caller-supplied PNG must not unlock verify. Include both source and host-captured visual IDs, with non-empty observations. Image registration checks bytes and dimensions; independently confirm the actual browser route, freshness, keyboard behavior, and fallback. A deliberately corrupt PNG must fail registration.
