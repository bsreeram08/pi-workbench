...
For visual work, start with `/plan --visual` (or `--ui`). That stores `surface: "visual"` on the plan. It is not `workflowMode`. Interactive `/plan` without the flag asks before entering the visual loop when the task looks like UI work; `/autopilot` never auto-enters.
